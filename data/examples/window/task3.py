...
df_window = df.withColumn('min_amt', F.min('purchase_amount').over(window))\
    .withColumn('max_amt', F.max('purchase_amount').over(window))

df_window.select('user_id', 'min_amt', 'max_amt').distinct().show(10, False)